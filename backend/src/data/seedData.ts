...
export const rawSeedData: RawCity[] = [
  // 1. France
  {
...
// Deterministic UUID generator for mock/seeded in-memory store
function generateDeterministicUUID(type: 'city' | 'activity', index: number): string {
  const hex = (index + 1).toString(16).padStart(12, '0');
  if (type === 'city') {
    return `11111111-1111-4000-8000-${hex}`;
  } else {
    return `22222222-2222-4000-8000-${hex}`;
  }
}
...
// Generate normalized Seed records
export const normalizedCities: City[] = [];
export const normalizedActivities: ActivityWithCity[] = [];

let actCounter = 0;
rawSeedData.forEach((rawCity, cityIdx) => {
  const cityId = generateDeterministicUUID('city', cityIdx);
  const cityRecord: City = {
    id: cityId,
    name: rawCity.name,
    country: rawCity.country,
    region: rawCity.region,
    cost_index: rawCity.cost_index,
    popularity: rawCity.popularity,
    image_url: rawCity.image_url,
    created_at: new Date('2026-01-01T00:00:00Z').toISOString(),
  };
  normalizedCities.push(cityRecord);

  rawCity.activities.forEach((rawAct) => {
    const actId = generateDeterministicUUID('activity', actCounter++);
    const actRecord: ActivityWithCity = {
      id: actId,
      city_id: cityId,
      name: rawAct.name,
      ...
      cost: rawAct.cost,
      duration_minutes: rawAct.duration_minutes,
      image_url: rawAct.image_url,
      created_at: new Date('2026-01-01T00:00:00Z').toISOString(),
      cities: cityRecord,
    };
    normalizedActivities.push(actRecord);
  });
});
